...
# ————————————————— Managed email (Resend via Emergent proxy) —————————————————
...
def set_auth_cookies(response: Response, user_id: str, email: str) -> None:
    response.set_cookie(key="access_token", value=create_access_token(user_id, email),
                        httponly=True, secure=True, samesite="none", max_age=900, path="/")
    response.set_cookie(key="refresh_token", value=create_refresh_token(user_id),
...
async def get_current_admin(request: Request) -> dict:
    ...
    user = await db.users.find_one({"_id": ObjectId(payload["sub"]), "role": "admin"})
    if not user:
...
def enquiry_email_html(doc: Enquiry) -> str:
    dashboard = f"{os.environ.get('FRONTEND_URL', '')}/admin"
    fields = [
        ("Name", doc.name),
        ("Email", doc.email),
    ...
    return (
        ...
        '<tr><td style="padding:0 32px 8px"><span style="font-size:11px;color:#5A6E64;text-transform:uppercase;letter-spacing:1px">Message</span>'
        f'<p style="font-size:14px;color:#121A16;line-height:1.6;background:#F9F6F0;border-radius:12px;padding:16px">{escape(doc.message)}</p></td></tr>'
        ...
        f'<p style="font-size:11px;color:#A3B8AD;margin:0">Sent by {escape(EMAIL_FROM_NAME)} — automated enquiry notification.</p>'
        '</td></tr>'
        '</table></td></tr></table>'
    )
...
async def create_enquiry(input: EnquiryCreate):
    ...
    try:
        await send_email(
            ...
            subject=f"New enquiry: {doc.category} - {doc.name}",
            html=enquiry_email_html(doc),
        )
    except Exception as exc:
        logger.error(f"Enquiry notification email failed: {exc}")
    return doc
...
async def login(input: LoginInput, request: Request, response: Response):
    ...
    now = time.time()
    if attempts and attempts.get("count", 0) >= 5 and now - attempts.get("last", 0) < 900:
